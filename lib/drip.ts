...
export async function sendDay1Followup({ to, name, from: fromCity, dest: toCity, requestId }: {
...
export async function sendDay3EmptyLeg({ to, name, from: fromCity, dest: toCity, legFrom, legTo, legDate, legPrice, legDiscount }: {
...
export async function sendDay7CaseStudy({ to, name, from: fromCity, dest: toCity, requestId }: {
  to: string; name: string; from: string; dest: string; requestId: string
}) {
  const html = base(`
...
// ─── Day 14: Last call ────────────────────────────────────────────────────────
export async function sendDay14LastCall({ to, name, from: fromCity, dest: toCity }: {
  to: string; name: string; from: string; dest: string
}) {
  const html = base(`
...
export async function runDripStep(step: DripStep, inquiry: {
  ...
  switch (step) {
    case 'day1':
      return sendDay1Followup({ to: email, name, from: fromCity, dest: toCity, requestId: id })
    case 'day3':
      return sendDay3EmptyLeg({
        to: email, name, from: fromCity, dest: toCity,
        legFrom: fromCity, legTo: toCity,
        legDate: 'Prossimi 7 giorni',
        legPrice: 4800, legDiscount: 58,
      })
    case 'day7':
      return sendDay7CaseStudy({ to: email, name, from: fromCity, dest: toCity, requestId: id })
    case 'day14':
      return sendDay14LastCall({ to: email, name, from: fromCity, dest: toCity })
  }
}
