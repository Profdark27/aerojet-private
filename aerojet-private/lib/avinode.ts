...
function generateMockAircraft(params: FlightSearchParams): Aircraft[] {
  ...
  // ── Realistic distance-based flight hours ──────────────────────────────────
  // Map common route pairs to realistic hours; fallback to estimate from city names
  const routeKey = `${params.fromICAO}-${params.toICAO}`
  const ROUTE_HOURS: Record<string, number> = {
    'LIML-EGLL': 2.1, 'EGLL-LIML': 2.1,
    'LIML-LFPB': 1.8, 'LFPB-LIML': 1.8,
    'LIRF-OMDB': 5.75, 'OMDB-LIRF': 5.75,
    'LIML-KJFK': 9.5, 'KJFK-LIML': 9.5,
    'LIMC-KTEB': 9.3, 'KTEB-LIMC': 9.3,
    'LSZH-LFMN': 0.75, 'LFMN-LSZH': 0.75,
    'LIPZ-LEIB': 2.5, 'LEIB-LIPZ': 2.5,
    'LIRF-UUDD': 3.75, 'UUDD-LIRF': 3.75,
    'LIML-LEMD': 2.8, 'LEMD-LIML': 2.8,
    'LIML-OMDB': 6.2, 'OMDB-LIML': 6.2,
    'LGMK-LIML': 2.3, 'LIML-LGMK': 2.3,
  }
  const flightHours = ROUTE_HOURS[routeKey] ?? (2.0 + Math.random() * 2.5)

  const basePricePerHour = { turboprop: 2800, light: 3900, midsize: 5800, supermid: 7200, heavy: 9800, ultralong: 14500 }

  // ── Aircraft definitions with CORRECT pax capacity checks ──────────────────
  const fmtTime = (h: number) => `${Math.floor(h)}h ${Math.round((h % 1) * 60)}m`
  ...
  const aircraft: Aircraft[] = [
    {
      ...
      currency: 'EUR',
      flightTime: fmtTime(flightHours),
    ...
    },
    {
      ...
      currency: 'EUR',
      flightTime: fmtTime(flightHours * 0.85),
    ...
    },
    {
      ...
      currency: 'EUR',
      flightTime: fmtTime(flightHours * 0.82),
    ...
    },
    {
      ...
      currency: 'EUR',
      flightTime: fmtTime(flightHours * 0.79),
    ...
    },
    {
      ...
      currency: 'EUR',
      flightTime: fmtTime(flightHours * 0.77),
    ...
    },
    {
      ...
      speed: 956,
      yearBuilt: 2024,
      price: Math.round(basePricePerHour.ultralong * flightHours * 1.08),
      currency: 'EUR',
      flightTime: fmtTime(flightHours * 0.76),
      amenities: ['Starlink WiFi', 'Chef di bordo', 'Suite master', 'Sala riunioni', 'Doccia separata', 'Cinema 4K'],
      rating: 5.0,
      available: pax <= 19,
      imageHint: 'gulfstream g700 interior',
    },
  ]
