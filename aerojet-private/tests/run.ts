...
async function run() {
  ...
  await test('sendRequestReceived: genera senza errori', async () => {
    const r = await sendRequestReceived({ to:'x@x.it', name:'Marco', from:'Milano', to:'Londra', date:'28 Apr', requestId:'RQ-TEST-001' })
    return (r as any).success === true
  })

  await test('sendBookingConfirmation: genera senza errori', async () => {
    const r = await sendBookingConfirmation({ to:'x@x.it', name:'Marco', aircraft:'Phenom 300E', from:'Milano', to:'Londra', date:'28 Apr', pax:2, deposit:2460, total:8200, confirmCode:'AJ-ABC123' })
    return (r as any).success === true
  })

  await test('sendQuoteToClient: genera senza errori', async () => {
    const r = await sendQuoteToClient({ to:'x@x.it', name:'Sofia', aircraft:'Falcon 7X', from:'Roma', to:'Dubai', date:'28 Apr', pax:6, price:48500, validUntil:'25 Apr', brokerName:'Corrado' })
    return (r as any).success === true
  })
  ...
  await test('Email drip Day1 follow-up: genera senza errori', async () => {
    const r = await sendDay1Followup({ to:'x@x.it', name:'Elena', from:'Venezia', to:'Ibiza', requestId:'RQ-005' })
    return (r as any).success === true
  })

  await test('Email drip Day7 case study: genera senza errori', async () => {
    const r = await sendDay7CaseStudy({ to:'x@x.it', name:'Roberto', from:'Ginevra', to:'Monaco', requestId:'RQ-006' })
    return (r as any).success === true
  })
