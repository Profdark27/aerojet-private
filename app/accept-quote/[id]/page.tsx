...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
    return (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 0', borderBottom: '1px solid rgba(201,168,76,0.06)' }}>
                  <span style={{ fontSize: 11, letterSpacing: 2, color: 'rgba(240,237,230,0.35)', fontFamily: 'Helvetica Neue, sans-serif', textTransform: 'uppercase' }}>{label}</span>span>
                  <span style={{ fontSize: 15, color: highlight ? '#C9A84C' : '#F0EDE6', fontFamily: 'Helvetica Neue, sans-serif', fontWeight: highlight ? 600 : 400 }}>{value}</span>span>
          </div>div>
        )
}
...
export default async function AcceptQuotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Wrap in try/catch to handle invalid Prisma IDs gracefully (returns 404 instead of crash)
  let quote
    try {
        ...
        <div style={{ minHeight: '100vh', background: '#0A0C14', color: '#F0EDE6', fontFamily: 'Georgia, serif', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, margin: '0 auto', padding: '48px 24px 80px' }}>
                          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none', marginBottom: 56 }}>
                                      <span style={{ color: '#C9A84C', fontSize: 18 }}>✦</span>span>
                                      <span style={{ fontSize: 20, fontWeight: 700, letterSpacing: 6, color: '#F0EDE6', fontFamily: 'Cormorant Garamond, Georgia, serif' }}>AEROJET</span>span>
                                      <span style={{ fontSize: 10, letterSpacing: 4, color: '#C9A84C', fontFamily: 'Helvetica Neue, sans-serif', alignSelf: 'flex-end', marginBottom: 2 }}>PRIVATE</span>span>
                          </Link>Link>

                  {destImage && (
                    <div style={{ position: 'relative', height: 220, marginBottom: 40, overflow: 'hidden', marginLeft: -24, marginRight: -24 }}>
                                  <ImageWithFallback src={destImage} alt={toCity} fill priority sizes="(max-width: 640px) 100vw, 640px" objectFit="cover" fallback={<div style={{ width: '100%', height: '100%', background: 'rgba(201,168,76,0.04)' }} />} />
                                  ...
                                  <div style={{ position: 'absolute', bottom: 20, left: 24, zIndex: 2 }}>
                                                  <div style={{ fontSize: 10, letterSpacing: 4, color: 'rgba(201,168,76,0.65)', fontFamily: 'Helvetica Neue, sans-serif', marginBottom: 4 }}>DESTINAZIONE</div>div>
                                                  <div style={{ fontSize: 22, fontWeight: 300, color: '#F0EDE6', letterSpacing: 3 }}>{toCity.toUpperCase()}</div>div>
                                  </div>div>
                    </div>div>
                  )}

                          <div style={{ marginBottom: 40 }}>
                                      <p style={{ fontSize: 11, letterSpacing: 3, color: '#C9A84C', fontFamily: 'Helvetica Neue, sans-serif', marginBottom: 12 }}>PREVENTIVO PERSONALIZZATO</p>p>
                                      <h1 style={{ fontSize: 'clamp(28px, 5vw, 42px)', fontWeight: 300, margin: '0 0 8px', letterSpacing: 1 }}>{fromCity} → {toCity}</h1>h1>
                                      <p style={{ fontSize: 14, color: 'rgba(240,237,230,0.4)', fontFamily: 'Helvetica Neue, sans-serif', margin: 0 }}>Gentile {inquiry.name}, il suo preventivo personalizzato è pronto.</p>p>
                          </div>div>

                  {alreadyPaid && (
                    <div style={{ background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.25)', padding: '14px 20px', marginBottom: 28 }}>
                                  <p style={{ color: '#4ade80', fontSize: 13, fontFamily: 'Helvetica Neue, sans-serif', margin: 0, letterSpacing: 1 }}>✓ Deposito già ricevuto — prenotazione in elaborazione</p>p>
                    </div>div>
                  )}
                  ...
                  {isExpired && !alreadyPaid && (
                    <div style={{ background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)', padding: '14px 20px', marginBottom: 28 }}>
                                  <p style={{ color: '#f87171', fontSize: 13, fontFamily: 'Helvetica Neue, sans-serif', margin: 0, letterSpacing: 1 }}>⚠ Preventivo scaduto il {validUntilStr} — contatta il concierge per un aggiornamento</p>p>
                    </div>div>
                  )}

                          <div style={{ background: '#0F1220', border: '1px solid rgba(201,168,76,0.15)', padding: '32px', marginBottom: 24 }}>
                                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                                                    <div style={{ fontSize: 10, letterSpacing: 3, color: '#C9A84C', fontFamily: 'Helvetica Neue, sans-serif' }}>DETTAGLI VOLO</div>div>
                                        {fleetImage && (
                        <div style={{ position: 'relative', width: 100, height: 60, overflow: 'hidden', flexShrink: 0, borderRadius: 4 }}>
                                          <ImageWithFallback src={fleetImage} alt={quote.aircraftModel} fill sizes="100px" objectFit="cover" fallback={<></>>} />
                        </div>div>
                                                  )}
                                      </div>div>
                        ...
                        <div style={{ background: '#0F1220', border: '1px solid rgba(201,168,76,0.2)', padding: '32px', marginBottom: 32 }}>
                                  <div style={{ fontSize: 10, letterSpacing: 3, color: '#C9A84C', fontFamily: 'Helvetica Neue, sans-serif', marginBottom: 24 }}>RIEPILOGO ECONOMICO</div>div>
                                  <div style={{ textAlign: 'center', paddingBottom: 28, marginBottom: 28, borderBottom: '1px solid rgba(201,168,76,0.1)' }}>
                                              <div style={{ fontSize: 11, letterSpacing: 2, color: 'rgba(240,237,230,0.35)', fontFamily: 'Helvetica Neue, sans-serif', marginBottom: 8 }}>PREZZO TOTALE CHARTER</div>div>
                                              <div style={{ fontSize: 44, fontWeight: 300, color: '#F0EDE6', letterSpacing: 1 }}>€{quote.price.toLocaleString('it-IT')}</div>div>
                                              <div style={{ fontSize: 11, color: 'rgba(240,237,230,0.25)', fontFamily: 'Helvetica Neue, sans-serif', marginTop: 4 }}>{quote.currency}</div>div>
                                  </div>div>
                                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 8 }}>
                                              <div style={{ background: 'rgba(201,168,76,0.06)', border: '1px solid rgba(201,168,76,0.15)', padding: '20px', textAlign: 'center' }}>
                                                            <div style={{ fontSize: 10, letterSpacing: 2, color: 'rgba(240,237,230,0.35)', fontFamily: 'Helvetica Neue, sans-serif', marginBottom: 8 }}>DEPOSITO (30%)</div>div>
                                                            <div style={{ fontSize: 26, color: '#C9A84C', fontWeight: 300 }}>€{depositAmount.toLocaleString('it-IT')}</div>
                                                            <div style={{ fontSize: 10, color: 'rgba(240,237,230,0.25)', fontFamily: 'Helvetica Neue, sans-serif', marginTop: 4 }}>Da pagare ora</div>div>
                                              </div>div>
                                              <div style={{ background: '#0A0C14', border: '1px solid rgba(201,168,76,0.08)', padding: '20px', textAlign: 'center' }}>
                                                            <div style={{ fontSize: 10, letterSpacing: 2, color: 'rgba(240,237,230,0.35)', fontFamily: 'Helvetica Neue, sans-serif', marginBottom: 8 }}>SALDO RESIDUO</div>div>
                                                            <div style={{ fontSize: 26, color: 'rgba(240,237,230,0.6)', fontWeight: 300 }}>€{remaining.toLocaleString('it-IT')}</div>div>
                                                            <div style={{ fontSize: 10, color: 'rgba(240,237,230,0.25)', fontFamily: 'Helvetica Neue, sans-serif', marginTop: 4 }}>Entro 72h dal volo</div>div>
                                              </div>div>
                                  </div>div>
                                  <p style={{ fontSize: 12, color: 'rgba(240,237,230,0.3)', fontFamily: 'Helvetica Neue, sans-serif', margin: '16px 0 0', lineHeight: 1.7 }}>Il prezzo include tutti i costi operativi, tasse aeroportuali e handling. Catering e servizi premium disponibili su richiesta al concierge.</p>p>
                        </div>div>
                
                  {!alreadyPaid && !isExpired && (
                    <div style={{ marginBottom: 32 }}>
                                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '16px 24px', marginBottom: 24 }}>
                                  {['🔒 Pagamento Protetto Stripe', '✈️ Operatori EASA / FAA', '🛡️ Rimborso Garantito'].map(trust => (
                                      <div key={trust} style={{ fontSize: 12, color: 'rgba(240,237,230,0.5)', fontFamily: 'Helvetica Neue, sans-serif', letterSpacing: 1, fontWeight: 500 }}>{trust}</div>div>
                                    ))}
                                </div>div>
                      ...
                      {/* PayButton client component — gestisce il checkout Stripe */}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, marginBottom: 40 }}>
                      ...
                      { n: '③', label: 'Volo confermato', sub: 'Documenti e briefing' },
                                    ...
                                    <div key={s.n} style={{ background: '#0F1220', border: '1px solid rgba(201,168,76,0.08)', padding: '16px 10px', textAlign: 'center' }}>
                                                    <div style={{ fontSize: 18, color: '#C9A84C', marginBottom: 6 }}>{s.n}</div>div>
                                                    <div style={{ fontSize: 11, letterSpacing: 1, color: '#F0EDE6', fontFamily: 'Helvetica Neue, sans-serif', marginBottom: 4, lineHeight: 1.3 }}>{s.label}</div>div>
                                                    <div style={{ fontSize: 10, color: 'rgba(240,237,230,0.3)', fontFamily: 'Helvetica Neue, sans-serif' }}>{s.sub}</div>div>
                                    </div>div>
                                  ))}
                    </div>div>
                        )}
                  ...
                  {alreadyPaid && (
                    <div style={{ textAlign: 'center', padding: '20px', background: 'rgba(74,222,128,0.05)', border: '1px solid rgba(74,222,128,0.15)' }}>
                                <p style={{ color: '#4ade80', fontFamily: 'Helvetica Neue, sans-serif', fontSize: 14, margin: 0 }}>Il suo deposito è stato ricevuto. Il concierge la contatterà entro 2 ore.</p>p>
                    </div>div>
                        )}
                  ...
                  {isExpired && !alreadyPaid && (
                    <a
                        ...
                        <p style={{ fontSize: 12, color: 'rgba(240,237,230,0.2)', fontFamily: 'Helvetica Neue, sans-serif', marginTop: 40, textAlign: 'center', lineHeight: 1.8 }}>
                                  Per assistenza: <a href={`mailto:${process.env.BROKER_EMAIL || 'concierge@aerojet.app'}`} style={{ color: '#C9A84C', textDecoration: 'none' }}>{process.env.BROKER_EMAIL || 'concierge@aerojet.app'}</a>a><br />
