...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!isValidId(id)) return { title: 'Preventivo Volo Privato' }
  
  try {
    const quote = await prisma.quote.findUnique({
      where: { id },
      select: { inquiry: { select: { fromCity: true, toCity: true } } },
    })
    if (quote?.inquiry) {
      const { fromCity, toCity } = quote.inquiry
      const route = fromCity && toCity ? `${fromCity} → ${toCity}` : 'Preventivo Volo Privato'
      return {
        title: route,
        openGraph: {
          title: `${route} | AeroJet Private`,
          images: [{ url: `/api/og?title=${encodeURIComponent(route)}&subtitle=Preventivo+Personalizzato`, width: 1200, height: 630 }],
        },
...
export default async function AcceptQuotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Validate ID format before hitting Prisma
  if (!isValidId(id)) {
    notFound()
  }
